...
const TRIGGER_OPTIONS = [
  ...
  { value: 'SHIPMENT_TRACKING_UPDATED', label: 'Shipment Tracking Updated' },
...
export default function AutomationRules() {
  ...
  // Dynamic carriers + services for dispatch.create_shipment macro
  const [dispatchCarriers, setDispatchCarriers] = useState<{ id: string; display_name: string }[]>([]);
  const [dispatchServices, setDispatchServices] = useState<{ code: string; name: string }[]>([]);
  const [dispatchServicesLoading, setDispatchServicesLoading] = useState(false);

  useEffect(() => {
    fetch(`${API_BASE}/dispatch/carriers/configured`, { headers: getApiHeaders(tenantId) })
      .then(r => r.ok ? r.json() : { carriers: [] })
      .then(d => setDispatchCarriers((d.carriers || []).filter((c: { is_active: boolean }) => c.is_active)))
      .catch(() => {});
  }, [tenantId]);

  useEffect(() => {
    const carrierId = String(editParameters['carrier_code'] ?? '');
    if (!editMacroType || !carrierId) { setDispatchServices([]); return; }
    setDispatchServicesLoading(true);
    fetch(`${API_BASE}/dispatch/carriers/${carrierId}/services`, { headers: getApiHeaders(tenantId) })
      .then(r => r.ok ? r.json() : { services: [] })
      .then(d => setDispatchServices(d.services || []))
      .catch(() => setDispatchServices([]))
      .finally(() => setDispatchServicesLoading(false));
  }, [editParameters['carrier_code'], editMacroType, tenantId]);
                ...
                <div style={{ padding: 16, overflowY: 'auto', flex: 1 }}>
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <select value={editMacroType} onChange={e => setEditMacroType(e.target.value)} style={{ width: '100%', padding: '6px 8px', borderRadius: 4, background: 'var(--bg-secondary,#1e293b)', border: '1px solid var(--border,#334155)', color: 'var(--text-primary,#e2e8f0)', fontSize: 13 }}>
                      ...
                      <option value="dispatch_create_shipment">Auto-Dispatch Shipment (Evri / carrier)</option>
                  ...
                  {editMacroType === 'dispatch_create_shipment' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                      <div>
                        <label style={{ display: 'block', fontSize: 12, color: 'var(--text-secondary,#94a3b8)', marginBottom: 3 }}>Carrier</label>
                        <select
                          value={String(editParameters['carrier_code'] ?? '')}
                          onChange={e => setEditParameters(p => ({ ...p, carrier_code: e.target.value, service_code: '' }))}
                          style={{ width: '100%', padding: '6px 8px', borderRadius: 4, background: 'var(--bg-secondary,#1e293b)', border: '1px solid var(--border,#334155)', color: 'var(--text-primary,#e2e8f0)', fontSize: 13 }}
                        >
                          <option value="">Select carrier…</option>
                          {dispatchCarriers.map(c => (
                            <option key={c.id} value={c.id}>{c.display_name}</option>
                          ))}
                          {dispatchCarriers.length === 0 && <option disabled>No carriers configured</option>}
                        </select>
                      </div>
                      {editParameters['carrier_code'] && (
                        <div>
                          <label style={{ display: 'block', fontSize: 12, color: 'var(--text-secondary,#94a3b8)', marginBottom: 3 }}>Service</label>
                          <select
                            value={String(editParameters['service_code'] ?? '')}
                            onChange={e => setEditParameters(p => ({ ...p, service_code: e.target.value }))}
                            disabled={dispatchServicesLoading}
                            style={{ width: '100%', padding: '6px 8px', borderRadius: 4, background: 'var(--bg-secondary,#1e293b)', border: '1px solid var(--border,#334155)', color: 'var(--text-primary,#e2e8f0)', fontSize: 13 }}
                          >
                            <option value="">{dispatchServicesLoading ? 'Loading…' : 'Select service…'}</option>
                            {dispatchServices.map(s => (
                              <option key={s.code} value={s.code}>{s.name}</option>
                            ))}
                            {!dispatchServicesLoading && dispatchServices.length === 0 && editParameters['carrier_code'] && (
                      ...
                      <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, cursor: 'pointer' }}>
                        <input
                          type="checkbox"
                          checked={Boolean(editParameters['include_return_label'])}
                          onChange={e => setEditParameters(p => ({ ...p, include_return_label: e.target.checked }))}
                        />
                        Generate return label simultaneously
                      </label>
                      <p style={{ fontSize: 11, color: 'var(--text-muted,#64748b)', margin: 0 }}>
                        Trigger: set rule trigger to <strong>Shipment Created</strong> or <strong>Order Created</strong> as appropriate.
                      </p>
                    </div>
                  )}
