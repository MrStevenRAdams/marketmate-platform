...
const BillingSettings: React.FC = () => {
  ...
  const [tab, setTab]         = useState<'overview' | 'usage' | 'audit'>('overview');
  ...
  return (
    <div className="bs-page">
      ...
      <div className="bs-tabs">
        {(['overview', 'usage', 'audit'] as const).map(t => (
          <button
            key={t}
            className={`bs-tab ${tab === t ? 'active' : ''}`}
            onClick={() => setTab(t)}
          >
            {{ overview: 'Overview', usage: 'Usage breakdown', audit: 'Activity log' }[t]}
          </button>
        ))}
      </div>
