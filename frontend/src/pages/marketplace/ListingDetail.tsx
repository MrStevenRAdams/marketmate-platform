...
export default function ListingDetail() {
  ...
  const [activeTab, setActiveTab] = useState<'details' | 'attributes' | 'raw' | 'downloaded' | 'performance' | 'seo'>(
    (searchParams.get('tab') as any) === 'seo' ? 'seo' : 'details'
  );
  ...
  useEffect(() => {
    // ListingDetail is a read-only fallback view.
    // Redirect to the /edit route which opens the channel-specific create form.
    // If the edit route can't determine the channel, it falls back here anyway.
    if (id) {
      navigate(`/marketplace/listings/${id}/edit`, { replace: true });
    }
  }, [id]);
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 20, borderBottom: '2px solid var(--border)' }}>
        {(['details', 'attributes', 'raw', 'downloaded', 'performance', 'seo'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {tab === 'details' ? 'Listing Details' : tab === 'attributes' ? `Attributes (${displayAttrs.length})` : tab === 'raw' ? 'Raw Data' : tab === 'downloaded' ? 'Downloaded Data' : tab === 'performance' ? '📊 Performance' : '🎯 SEO'}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'seo' && (
        <SEOScorePanel
          ...
          productAsin={listing.asin ?? null}
          productId={listing.product_id}
          listingChannel={listing.channel ?? 'amazon'}
          currentContent={{
            title,
            bullets: Array.isArray(bullets)
              ? bullets.map((b: any) => (typeof b === 'string' ? b : b?.value ?? '')).filter(Boolean)
              : [],
            description,
          }}
        />
