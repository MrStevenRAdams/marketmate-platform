...
import { truncateToEtsyTag, fetchKeywordIntelligence } from '../../utils/keywordUtils';
import { KeywordIntelligenceResponse } from '../../types/seo';
import { LayerStatusBadge } from '../../components/seo/LayerStatusBadge';
...
export default function EtsyListingCreate() {
  ...
  // Session 8: fetch keyword intelligence when productId is available
  useEffect(() => {
    if (!productId) return;
    setKwLoading(true);
  ...
  // Session 8: add a suggested keyword as a tag (truncated to 20 chars)
  function addSuggestedTag(keyword: string) {
    const tag = truncateToEtsyTag(keyword);
    if (!tag || tags.includes(tag) || tags.length >= 13) return;
          ...
          <div className="card">
            ...
            <div className="card-body space-y-4">
              ...
              <div>
                ...
                {/* ── Suggested tags (Session 8) ── */}
                {productId && (
                  <div style={{ marginTop: 12, padding: '10px 12px', background: 'var(--bg-secondary)', borderRadius: 8, border: '1px solid var(--border)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>Suggested tags</span>
                      <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>{tags.length}/13 tags used</span>
                      {kwData && <LayerStatusBadge layer={kwData.source_layer as any} />}
                    </div>
                    {kwLoading && <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>Loading keyword suggestions…</p>}
                    {!kwLoading && !kwData && <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>No keyword data available for this product yet.</p>}
                    {!kwLoading && kwData && (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                        {kwData.keywords.slice(0, 13).map((entry) => {
                          const tag = truncateToEtsyTag(entry.keyword);
                          const alreadyAdded = tags.includes(tag);
                          const limitReached = tags.length >= 13;
                          return (
                            <span key={entry.keyword} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 20, fontSize: 11, background: alreadyAdded ? 'var(--bg-primary)' : 'rgba(20,184,166,0.1)', border: `1px solid ${alreadyAdded ? 'var(--border)' : 'rgba(20,184,166,0.4)'}`, color: alreadyAdded ? 'var(--text-muted)' : 'var(--text-primary)', opacity: limitReached && !alreadyAdded ? 0.45 : 1 }}>
                              {tag}
                              {!alreadyAdded && (
                                <button onClick={() => addSuggestedTag(entry.keyword)} disabled={limitReached} title="Add tag" style={{ background: 'none', border: 'none', cursor: limitReached ? 'not-allowed' : 'pointer', padding: 0, lineHeight: 1, color: 'rgb(20,184,166)', fontSize: 13, fontWeight: 700 }}>+</button>
                              )}
                              {alreadyAdded && <span style={{ color: 'rgb(20,184,166)', fontSize: 10 }}>✓</span>}
                            </span>
                          );
                        })}
                      </div>
