...
// Convert pence string from API to currency string (e.g. "1299" -> "12.99")
const penceToCurrency = (pence: string): string => {
  const p = parseInt(pence);
  if (isNaN(p)) return pence;
  return (p / 100).toFixed(2);
};

// Convert currency string to pence string (e.g. "12.99" -> "1299")
const currencyToPence = (currency: string): string => {
  const f = parseFloat(currency);
  if (isNaN(f)) return '0';
    ...
    try {
      ...
      if (d) {
        ...
        const draftObj: any = {
          ...
          sku: d.sku || '',
          retailPrice: d.price?.baseAmount ? penceToCurrency(d.price.baseAmount) : '',
          listPrice: d.price?.listAmount ? penceToCurrency(d.price.listAmount) : '',
          currency,
          images: d.images || [],
      ...
      if (aiFlag === 'pending' && data.draft && data.template) {
        ...
        try {
          ...
          let goodsProps: any[] = [];
          for (const path of [['goodsProperties'], ['templateInfo', 'goodsProperties'], ['template', 'goodsProperties']]) {
      ...
      if (d?.variants && d.variants.length > 0) {
        ...
        const fallbackPrice = d.price?.baseAmount ? penceToCurrency(d.price.baseAmount) : '';
        const loadedVariants: TemuVariant[] = d.variants.map(v => ({
          ...v,
          retailPrice: v.price || fallbackPrice,
          listPrice: '',
        }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await temuApi.submit({
        ...
        // Convert currency back to pence for the API
        price: { baseAmount: currencyToPence(draft.retailPrice), listAmount: draft.listPrice ? currencyToPence(draft.listPrice) : undefined, currency: draft.currency },
    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <Section title="Title *">
          ...
          <small style={{ color: draft.title.length > 450 ? 'var(--danger)' : 'var(--text-tertiary)' }}>{draft.title.length}/500</small>
        </Section>
