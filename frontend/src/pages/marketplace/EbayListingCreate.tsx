...
export default function EbayListingCreate() {
  ...
  // Session 8: find a keyword suggestion matching an item specific field name
  function getSuggestionForAspect(aspectName: string): string | null {
    if (!kwData?.keywords) return null;
    const name = aspectName.toLowerCase();
    const match = kwData.keywords.find(entry => {
      const kw = entry.keyword.toLowerCase();
      return kw.includes(name) || name.includes(kw);
    });
          ...
          {aspectsLoading ? <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>Loading item specifics...</p> : itemAspects.length > 0 && (
            <div>
              ...
              {requiredAspects.length > 0 && <div style={{ marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {requiredAspects.map(a => {
                    const suggestion = getSuggestionForAspect(a.localizedAspectName);
                    const isEmpty = !getAspectValue(a.localizedAspectName);
                    return (
                      <div key={a.localizedAspectName}>
                        <AspectField aspect={a} value={getAspectValue(a.localizedAspectName)} values={draft.aspects[a.localizedAspectName] || []} onChange={vals => updateAspect(a.localizedAspectName, vals)} tag="REQ" tagColor="var(--warning)" />
                        {isEmpty && suggestion && (
                          <div style={{ marginTop: -4, marginBottom: 4, paddingLeft: 40 }}>
                            <button onClick={() => updateAspect(a.localizedAspectName, [suggestion])} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 11, color: 'rgb(20,184,166)' }}>
                              Suggested: <strong>{suggestion}</strong> — click to apply
                            </button>
                          </div>
                        )}
                      </div>
              ...
              {recommendedAspects.length > 0 && <Subsection title={`Recommended (${recommendedAspects.length})`} defaultOpen={false}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {recommendedAspects.map(a => {
                    const suggestion = getSuggestionForAspect(a.localizedAspectName);
                    const isEmpty = !getAspectValue(a.localizedAspectName);
                    return (
                      <div key={a.localizedAspectName}>
                        <AspectField aspect={a} value={getAspectValue(a.localizedAspectName)} values={draft.aspects[a.localizedAspectName] || []} onChange={vals => updateAspect(a.localizedAspectName, vals)} tag="REC" tagColor="var(--primary-light)" />
                        {isEmpty && suggestion && (
                          <div style={{ marginTop: -4, marginBottom: 4, paddingLeft: 40 }}>
                            <button onClick={() => updateAspect(a.localizedAspectName, [suggestion])} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 11, color: 'rgb(20,184,166)' }}>
                              Suggested: <strong>{suggestion}</strong> — click to apply
              ...
              {optionalAspects.length > 0 && <Subsection title={`Optional (${optionalAspects.length})`} defaultOpen={false}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {optionalAspects.map(a => {
                    ...
                    return (
                      <div key={a.localizedAspectName}>
                        <AspectField aspect={a} value={getAspectValue(a.localizedAspectName)} values={draft.aspects[a.localizedAspectName] || []} onChange={vals => updateAspect(a.localizedAspectName, vals)} tag="OPT" tagColor="var(--text-muted)" />
                        {isEmpty && suggestion && (
                          <div style={{ marginTop: -4, marginBottom: 4, paddingLeft: 40 }}>
                            <button onClick={() => updateAspect(a.localizedAspectName, [suggestion])} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 11, color: 'rgb(20,184,166)' }}>
                              Suggested: <strong>{suggestion}</strong> — click to apply
