...
import { assembleMetaDescription, fetchKeywordIntelligence } from '../../utils/keywordUtils';
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '20px 16px 80px' }}>
      ...
      <Section title="SEO" accent="#06b6d4">
        ...
        <div>
          {/* Session 8: enhanced meta description with keyword generation */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <label style={{ ...labelStyle, marginBottom: 0 }}>Meta Description (Google Shopping)</label>
            {kwData && <LayerStatusBadge layer={kwData.source_layer as any} />}
            {kwData && productId && (
              <button onClick={() => { const kws = kwData.keywords.map(e => e.keyword); const meta = assembleMetaDescription(kws, draft.title || draft.seoTitle || ''); updateDraft('seoDescription', meta); }} style={{ marginLeft: 'auto', fontSize: 11, padding: '2px 8px', borderRadius: 6, border: '1px solid rgba(20,184,166,0.5)', background: 'rgba(20,184,166,0.08)', color: 'rgb(20,184,166)', cursor: 'pointer' }}>
                ✨ Generate
              </button>
            )}
          </div>
          <textarea value={draft.seoDescription} onChange={e => updateDraft('seoDescription', e.target.value)} style={{ ...textareaStyle, minHeight: 80 }} placeholder="Appears in Google search results. Front-load the primary keyword." maxLength={320} />
          <div style={{ fontSize: 11, marginTop: 3, display: 'flex', gap: 8, color: draft.seoDescription.length > 155 ? '#fbbf24' : 'var(--text-muted)' }}>
            <span>{draft.seoDescription.length}/320 chars</span>
            {draft.seoDescription.length > 0 && draft.seoDescription.length <= 155 && <span style={{ color: 'rgb(20,184,166)' }}>✓ within 155-char Google limit</span>}
            {draft.seoDescription.length > 155 && <span style={{ color: '#fbbf24' }}>⚠ over 155-char Google Shopping recommendation</span>}
          </div>
          <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>Appears in Google search results. Front-load the primary keyword. Aim for ≤155 chars.</p>
        </div>
      </Section>
