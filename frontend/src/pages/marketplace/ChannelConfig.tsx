...
interface ChannelConfig {
  ...
  products: { import_enabled: boolean };
}
...
const TABS = [
  { id: 'products_import', icon: '📥', label: 'Product Import' },
...
function ProductImportTab({ config, onChange, credentialId, channel }: {
  ...
  const importEnabled = config.products?.import_enabled ?? true;
  ...
  async function handleImportNow() {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/marketplace/import`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!res.ok) {
        setImportResult(`❌ ${data.error || `HTTP ${res.status}`}`);
        return;
      }
      // Response shape: { data: { job_id: "..." }, message: "Import job started" }
      const jobId = data.data?.job_id || data.job_id;
      setImportResult(`✅ Import started${jobId ? ` (Job: ${String(jobId).slice(0, 8)}...)` : ''}`);
    } catch (e: any) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <Toggle
        ...
        onChange={v => onChange({ ...config, products: { ...config.products, import_enabled: v } })}
        label="Auto-import Products"
        description="Automatically download products from this channel into your catalogue when the account is connected or activated. On by default."
      />

      <div style={{ borderTop: '1px solid var(--border)', paddingTop: 24 }}>
        <SectionLabel>Import Now</SectionLabel>
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
          Manually trigger a product import from this channel. New products not already in your catalogue or review queue will be downloaded immediately.
        </p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button
            onClick={handleImportNow}
            disabled={importing || !credentialId}
            style={{
          ...
          >
            {importing ? '⏳ Importing...' : '📥 Import Products Now'}
          </button>
          {importResult && (
            <span style={{ fontSize: 13, color: importResult.startsWith('✅') ? 'var(--success, #10b981)' : 'var(--danger, #ef4444)' }}>
              {importResult}
            </span>
          )}
        </div>
...
export default function ChannelConfig() {
  ...
  const [tab, setTab] = useState<TabId>('products_import');
    ...
    try {
      ...
      if (cfgRes.status === 'fulfilled' && cfgRes.value.ok) {
        ...
        setConfig({
          ...
          products: { ...DEFAULT_CONFIG.products, ...(rawCfg.products || {}) },
          ...
          temu_defaults: rawCfg.temu_defaults
            ? { ...DEFAULT_CONFIG.temu_defaults, ...rawCfg.temu_defaults }
            : DEFAULT_CONFIG.temu_defaults,
        });
      }
      ...
      <div style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', display: 'flex', minHeight: 560 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, padding: 28, overflowY: 'auto' }}>
            {tab === 'products_import' && <ProductImportTab config={config} onChange={setConfig} credentialId={id} channel={channelName} />}
