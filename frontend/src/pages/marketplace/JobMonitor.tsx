...
function CustomerJobsTab() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    setLoadingJobs(true);
    try {
      const targets = selectedTenant === 'ALL'
        ? tenants.map(t => t.tenant_id)
        : [selectedTenant];

      const results = await Promise.allSettled(
        targets.map(tid =>
          fetch(`${API_BASE}/marketplace/import/jobs?page_size=100`, {
            headers: { 'X-Tenant-Id': tid },
          })
          .then(r => r.json())
          .then(d => (d.data || []).map((j: any) => ({
            ...j,
            _tenantId:   tid,
            _tenantName: tenants.find(t => t.tenant_id === tid)?.name || tid,
          })))
          .catch(() => [])
        )
      );

      const flat: any[] = results.flatMap(r => r.status === 'fulfilled' ? r.value : []);
      flat.sort((a, b) => {
