...
import { KeywordIntelligenceCard } from '../components/seo/KeywordIntelligenceCard';
import {
  BatchesTab,
...
export default function ProductCreate() {
  ...
  const [activeTab, setActiveTab] = useState<string>(() => {
    // Allow deep-linking to a specific tab via ?tab=listings etc.
  ...
  const [productAsin, setProductAsin] = useState<string | null>(null); // always tracks ASIN regardless of primary identifier
  ...
  // Shipping & Dispatch fields (wired to product model shipping fields)
  const [shippingLabelsPerShipment, setShippingLabelsPerShipment] = useState(1);
  const [shippingDefaultCarrier, setShippingDefaultCarrier] = useState('');
  const [shippingDefaultService, setShippingDefaultService] = useState('');
  const [shippingIncludeReturn, setShippingIncludeReturn] = useState(false);
  const [shippingTemplate, setShippingTemplate] = useState('');
  const [shippingCustomsProfileId, setShippingCustomsProfileId] = useState('');

  // Dynamic data for shipping dropdowns
  const [shippingCarriers, setShippingCarriers] = useState<{ id: string; display_name: string }[]>([]);
  const [shippingCarrierServices, setShippingCarrierServices] = useState<{ code: string; name: string }[]>([]);
  const [shippingCarrierServicesLoading, setShippingCarrierServicesLoading] = useState(false);
  const [shippingTemplates, setShippingTemplates] = useState<{ id: string; name: string; layout: string }[]>([]);
  const [shippingCustomsProfiles, setShippingCustomsProfiles] = useState<{ id: string; name: string; commodity_code: string; duty_paid: string }[]>([]);
  ...
  // Reload carrier services when the selected default carrier changes
  useEffect(() => {
    if (!shippingDefaultCarrier) { setShippingCarrierServices([]); return; }
    const base = import.meta.env.VITE_API_URL ?? 'http://localhost:8080/api/v1';
    setShippingCarrierServicesLoading(true);
    fetch(`${base}/dispatch/carriers/${shippingDefaultCarrier}/services`, {
      headers: { 'X-Tenant-Id': getActiveTenantId() },
    })
      .then(r => r.ok ? r.json() : { services: [] })
      .then(d => setShippingCarrierServices(d.services || []))
      .catch(() => setShippingCarrierServices([]))
      .finally(() => setShippingCarrierServicesLoading(false));
  ...
  async function loadShippingDropdownData() {
    const base = import.meta.env.VITE_API_URL ?? 'http://localhost:8080/api/v1';
    const headers = { 'X-Tenant-Id': getActiveTenantId() };
    // Load configured carriers
    try {
      const res = await fetch(`${base}/dispatch/carriers/configured`, { headers });
      if (res.ok) {
        const data = await res.json();
        setShippingCarriers((data.carriers || []).filter((c: any) => c.is_active));
      }
    } catch { /* silently ignore */ }
    // Load shipping templates
    try {
      const res = await fetch(`${base}/dispatch/shipping-templates`, { headers });
      if (res.ok) {
        const data = await res.json();
    ...
    try {
      const res = await fetch(`${base}/dispatch/customs-profiles`, { headers });
      if (res.ok) {
        const data = await res.json();
        setShippingCustomsProfiles(data.profiles || []);
      }
    } catch { /* silently ignore */ }
  }
  ...
  async function loadProduct(productId: string) {
    ...
    try {
      ...
      if (p.identifiers) {
        ...
        // Always track ASIN separately — used for keyword intelligence regardless of primary identifier
        if (ids.asin) setProductAsin(ids.asin);
      }
      ...
      setShippingLabelsPerShipment(p.labels_per_shipment || 1);
      setShippingDefaultCarrier(p.default_carrier || '');
      setShippingDefaultService(p.default_service || '');
      setShippingIncludeReturn(p.include_return_label || false);
      setShippingTemplate(p.shipping_template || '');
      setShippingCustomsProfileId(p.customs_profile_id || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const productData: any = {
        ...
        // Shipping & Dispatch fields
        labels_per_shipment: shippingLabelsPerShipment || 1,
        default_carrier: shippingDefaultCarrier || null,
        default_service: shippingDefaultService || null,
        include_return_label: shippingIncludeReturn,
        shipping_template: shippingTemplate || null,
        customs_profile_id: shippingCustomsProfileId || null,
      };
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit} onChange={() => { if (!isDirty) setIsDirty(true); }}>
            ...
            <div className="card" style={{ marginBottom: 'var(--spacing-lg)' }}>
              ...
              <div style={{ padding: 'var(--spacing-lg)' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  {/* Labels per Shipment + Include Return Label */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div>
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px', fontWeight: 500 }}>Labels per Shipment</div>
                      <input
                        type="number"
                        className="input"
                        style={{ width: '100%' }}
                        min={1}
                        max={20}
                        value={shippingLabelsPerShipment}
                        onChange={e => setShippingLabelsPerShipment(Math.max(1, Math.min(20, parseInt(e.target.value) || 1)))}
                      />
                      <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '3px' }}>
                        For multi-box shipments. Each parcel gets a unique barcode.
                      </div>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', paddingTop: '18px' }}>
                      <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '13px', color: 'var(--text-secondary)' }}>
                        <input
                          type="checkbox"
                          checked={shippingIncludeReturn}
                          onChange={e => setShippingIncludeReturn(e.target.checked)}
                        />
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <div>
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px', fontWeight: 500 }}>Default Carrier</div>
                      <select
                        className="select"
                        style={{ width: '100%' }}
                        value={shippingDefaultCarrier}
                        onChange={e => { setShippingDefaultCarrier(e.target.value); setShippingDefaultService(''); }}
                      >
                        ...
                        {shippingCarriers.length === 0 && (
                          <option disabled>No carriers configured</option>
                        )}
                      </select>
                    </div>
                    <div>
                      <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px', fontWeight: 500 }}>Default Service</div>
                      <select
                        className="select"
                        style={{ width: '100%' }}
                        value={shippingDefaultService}
                        onChange={e => setShippingDefaultService(e.target.value)}
                        disabled={!shippingDefaultCarrier || shippingCarrierServicesLoading}
                      >
                        <option value="">
                          {!shippingDefaultCarrier ? '— Select a carrier first —' : shippingCarrierServicesLoading ? 'Loading…' : '— No preference —'}
                        </option>
                        {shippingCarrierServices.map(s => (
                  ...
                  {/* Shipping Template */}
                  <div>
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px', fontWeight: 500 }}>Shipping Template</div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      <a
                        href="/dispatch/shipping-templates"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="btn btn-secondary"
                        style={{ fontSize: '12px', padding: '6px 12px', whiteSpace: 'nowrap', textDecoration: 'none' }}
                      >
                        + New
                      </a>
                    </div>
                  ...
                  {/* Customs Profile */}
                  <div>
                    <div style={{ fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px', fontWeight: 500 }}>Customs Profile</div>
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <select
                        className="select"
                        style={{ flex: 1 }}
                        value={shippingCustomsProfileId}
                        onChange={e => setShippingCustomsProfileId(e.target.value)}
                      >
                        <option value="">— None —</option>
                        {shippingCustomsProfiles.map(p => (
                      ...
                      <a
                        href="/dispatch/customs-profiles"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="btn btn-secondary"
                        style={{ fontSize: '12px', padding: '6px 12px', whiteSpace: 'nowrap', textDecoration: 'none' }}
                    ...
                    <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '3px' }}>
                      For international shipments — HS code, duty terms, country of manufacture.
                    </div>
                  </div>
        ...
        {/* Keyword Intelligence — only shown for existing saved products */}
