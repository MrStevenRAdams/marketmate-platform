...
const CARRIERS = ['evri', 'royal_mail', 'dpd', 'ups', 'fedex', 'dhl'];
...
export default function Workflows() {
  ...
  useEffect(() => {
    ...
    loadCredentials();
  }, [loadWorkflows, loadFulfilmentSources, loadCredentials]);
    ...
    <div className="wf-page">
      <div className="wf-header">
        ...
        <div className="wf-header-actions">
          <button className="btn btn-secondary" onClick={() => navigate('/dev/workflow-simulator')}>
            🧪 Simulator
          </button>
...
function WorkflowEditor({ form, setForm, isNew, saving, saveError, fulfilmentSources, credentials,
...
function ActionRow({ idx, action, fulfilmentSources, onChange, onRemove }: {
    ...
    <div className="wf-condition-row wf-action-row">
      ...
      <div className="wf-condition-fields">
        ...
        {action.type === 'assign_carrier' && (
          <div className="wf-inline-fields">
            <select className="wf-select-sm" value={action.carrier_id || ''} onChange={e => onChange({ carrier_id: e.target.value })}>
              <option value="">— Carrier —</option>
              {CARRIERS.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <input className="wf-input-sm" placeholder="Service code (e.g. ND24)" value={action.service_code || ''}
