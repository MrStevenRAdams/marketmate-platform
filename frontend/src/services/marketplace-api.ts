...
export interface ListingSearchParams {
  q?: string;
  channel?: string;
  state?: string;           // comma-separated for OR: "error,blocked"
  brand?: string;
  category?: string;
  account_name?: string;
  channel_account_id?: string;
...
export interface ListingSearchFacetCount {
  field_name: string;
...
export interface ListingSearchResult {
  ...
  page: number;
  out_of: number;
  facet_counts?: ListingSearchFacetCount[];
}

export const searchService = {
  /** GET /search/listings — Typesense-backed listing search with facets */
  searchListings: (params: ListingSearchParams) =>
    apiClient.get<ListingSearchResult>('/search/listings', { params }),

  /** POST /search/reindex-listings — drop+recreate collection + reindex all */
  reindexListings: () =>
    apiClient.post<{ ok: boolean; indexed: number; message: string }>('/search/reindex-listings'),
};
