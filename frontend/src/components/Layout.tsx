...
export default function Layout() {
  ...
  const defaultModules: EnabledModules = {
    wms: true, advanced_dispatch: true, purchase_orders: true,
    automation: true, rma: true, advanced_analytics: true, email_system: true,
  };
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/settings/modules`, { headers: { 'X-Tenant-Id': tenantId } })
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (data?.modules) setModules({ ...defaultModules, ...data.modules });
      })
      .catch(() => {}); // Falls back to all-enabled defaults
  }, [activeTenant]);
