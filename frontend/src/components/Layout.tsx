...
export default function Layout() {
  ...
  // MVP defaults — WMS, PurchaseOrders, RMA, AdvancedAnalytics hidden until enabled per-tenant
  const defaultModules: EnabledModules = {
    wms: false, advanced_dispatch: false, purchase_orders: false,
    automation: true, rma: false, advanced_analytics: false, email_system: true,
  };
  const [modules, setModules] = useState<EnabledModules>(defaultModules);

  // Collapsible nav sections — track which are expanded
  const getInitialExpanded = (): Record<string, boolean> => {
    try {
      const stored = localStorage.getItem('nav_expanded');
  ...
  const toggleSection = (key: string) => {
    setExpanded(prev => {
      ...
      try { localStorage.setItem('nav_expanded', JSON.stringify(next)); } catch {}
      return next;
    });
  };
  ...
  const sectionForPath = (path: string): string | null => {
    if (path.startsWith('/products') || path === '/categories' || path === '/attributes') return 'catalog';
    if (path.startsWith('/marketplace/import') || path.startsWith('/marketplace/listings') || path.startsWith('/marketplace/configurators')) return 'marketplace';
    if (path === '/orders' || path.startsWith('/messages') || path.startsWith('/import-export') || path.startsWith('/purchase-orders') || path.startsWith('/suppliers') || path.startsWith('/vendor-orders')) return 'operations';
    if (path === '/dispatch' || path.startsWith('/dispatch/') || path === '/pickwaves' || path === '/manifests') return 'dispatch';
    if (path.startsWith('/my-inventory') || path === '/inventory' || path === '/warehouse-locations' || path === '/storage-groups' || path === '/stock-count' || path === '/stock-in' || path === '/stock-scrap' || path === '/warehouse-transfers' || path === '/picking-replenishment' || path.startsWith('/stock') || path === '/forecasting' || path === '/replenishment' || path === '/fulfilment-sources' || path.startsWith('/marketplace/fba-inbound')) return 'inventory';
    if (path.startsWith('/rmas')) return 'returns';
    if (path.startsWith('/workflows') || path === '/automation-rules' || path === '/workflow-simulator' || path === '/automation-logs') return 'automation';
    if (path.startsWith('/analytics') || path === '/reports') return 'analytics';
    return null;
  };

  useEffect(() => {
    const section = sectionForPath(location.pathname);
    if (section && !expanded[section]) {
      setExpanded(prev => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const r = await fetch(`${API_BASE}/settings/modules`, {
          headers: { 'X-Tenant-Id': tenantId, 'Authorization': `Bearer ${token}` }
        });
        if (!r.ok) return;
        const data = await r.json();
        if (data?.modules) setModules({ ...defaultModules, ...data.modules });
      } catch {}
    };
    load();
  }, [activeTenant]);
  ...
  const [temuWizardStage, setTemuWizardStage] = useState<string>('');

  useEffect(() => {
    if (!activeTenant) return;
    const tenantId = activeTenant?.tenant_id || '';
    fetch(`${API_BASE}/settings/setup-status`, { headers: { 'Content-Type': 'application/json', 'X-Tenant-Id': tenantId } })
      .then(r => {
        if (!r.ok) return { setup_complete: true }; // Endpoint missing or error — don't block
        return r.json();
      })
      .then(data => {
        setSetupComplete(data.setup_complete === true); // Explicit true check — false and undefined both mean not complete
        if (data.referral_source) setReferralSource(data.referral_source);
        if (data.temu_wizard_stage) setTemuWizardStage(data.temu_wizard_stage);
      })
      .catch(() => setSetupComplete(true)); // Network error — fail open
  }, [activeTenant]);
  ...
  // Fetch pending review count for nav badge — refreshes on route change
  useEffect(() => {
    if (!activeTenant) return;
    const tenantId = activeTenant?.tenant_id || '';
    fetch(`${API_BASE}/marketplace/pending-review/count`, { headers: { 'X-Tenant-Id': tenantId } })
      .then(r => r.ok ? r.json() : null)
  ...
  const resolveAmazonCred = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Check for Amazon Vendor account
        const vendorCred = (Array.isArray(creds) ? creds : []).find(
          (c: any) => (c.channel === 'amazon_vendor' || c.account_type === 'vendor') && c.active !== false
        );
        if (vendorCred) {
  ...
  // Wait for setup status check before rendering — but only if we have a tenant
  // If activeTenant is null, setupComplete stays null, so skip the check entirely
  if (activeTenant && setupComplete === null) {
    return (
      <div style={{
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        height: '100vh', background: 'var(--bg-primary)',
  ...
  // Redirect to setup wizard if onboarding not complete
  if (setupComplete === false) {
    // Temu referrals bypass the generic setup wizard and go straight to the Temu wizard
    if (referralSource === 'temu') {
      return <Navigate to="/temu-wizard" replace />;
    }
    return <Navigate to="/setup" replace />;
  }

  // Even if setup is complete, check if Temu wizard is in progress
  if (referralSource === 'temu' && temuWizardStage && temuWizardStage !== 'completed') {
    return <Navigate to="/temu-wizard" replace />;
  }
    ...
    <div className="app-container">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          {/* ── CATALOG ──────────────────────────────────────────────── */}
          <div className="nav-section">
            <button className="nav-section-title nav-section-toggle" onClick={() => toggleSection('catalog')}>
              <span>CATALOG</span>
              <span className="nav-toggle-arrow">{expanded['catalog'] ? '▾' : '›'}</span>
            </button>
            {expanded['catalog'] && (<>
              <Link to="/products" className={`nav-item ${isActivePrefix('/products') && !isActive('/products/review-mappings') ? 'active' : ''}`}>
                <span className="nav-icon">📦</span><span>Products</span>
              </Link>
              <Link to="/products/review-mappings" className={`nav-item ${isActive('/products/review-mappings') ? 'active' : ''}`}>
                <span className="nav-icon">🔍</span><span>Review Mappings</span>
                {pendingReviewCount > 0 && (
                  <span style={{ marginLeft: 'auto', background: '#f59e0b', color: '#fff', borderRadius: '999px', fontSize: '11px', fontWeight: 700, padding: '1px 7px', lineHeight: '16px' }}>
                    {pendingReviewCount}
                  </span>
                )}
              </Link>
          ...
          {/* ── MARKETPLACE ──────────────────────────────────────────── */}
          {/* Connections moved to Settings. Extract Inventory merged into Import. */}
          {/* Configurators accessed via button on Listings page. */}
          {/* Price Sync now a per-channel setting. */}
          <div className="nav-section">
            <button className="nav-section-title nav-section-toggle" onClick={() => toggleSection('marketplace')}>
              <span>MARKETPLACE</span>
              <span className="nav-toggle-arrow">{expanded['marketplace'] ? '▾' : '›'}</span>
            </button>
            {expanded['marketplace'] && (<>
              <Link to="/marketplace/import" className={`nav-item ${isActivePrefix('/marketplace/import') ? 'active' : ''}`}>
                <span className="nav-icon">⬇️</span><span>Import Products</span>
              </Link>
          ...
          {/* ── OPERATIONS (renamed from Orders) ─────────────────────── */}
          <div className="nav-section">
            <button className="nav-section-title nav-section-toggle" onClick={() => toggleSection('operations')}>
              <span>OPERATIONS</span>
              <span className="nav-toggle-arrow">{expanded['operations'] ? '▾' : '›'}</span>
            </button>
            {expanded['operations'] && (<>
              <Link to="/orders" className={`nav-item ${isActive('/orders') ? 'active' : ''}`}>
                <span className="nav-icon">🛒</span><span>Orders</span>
              </Link>
              ...
              {hasVendorAccount && (
                <Link to="/vendor-orders" className={`nav-item ${isActivePrefix('/vendor-orders') ? 'active' : ''}`}>
          ...
          {/* ── DISPATCH (module-gated) ───────────────────────────────── */}
          {/* Workflow: Pickwaves → Despatch Console → Label Printing → Manifests */}
          {/* Shipping/Packaging Rules moved to Settings. */}
          {modules.advanced_dispatch && (
          <div className="nav-section">
            <button className="nav-section-title nav-section-toggle" onClick={() => toggleSection('dispatch')}>
              <span>DISPATCH</span>
              <span className="nav-toggle-arrow">{expanded['dispatch'] ? '▾' : '›'}</span>
            </button>
            {expanded['dispatch'] && (<>
              ...
              <Link to="/dispatch/sla-dashboard" className={`nav-item ${isActive('/dispatch/sla-dashboard') ? 'active' : ''}`}>
                <span className="nav-icon">⏱️</span><span>SLA Dashboard</span>
              </Link>
              <Link to="/dispatch/exceptions" className={`nav-item ${isActive('/dispatch/exceptions') ? 'active' : ''}`}>
                <span className="nav-icon">⚠️</span><span>Delivery Exceptions</span>
              </Link>
              <Link to="/dispatch/shipping-templates" className={`nav-item ${isActive('/dispatch/shipping-templates') ? 'active' : ''}`}>
                <span className="nav-icon">📐</span><span>Shipping Templates</span>
              </Link>
              <Link to="/dispatch/customs-profiles" className={`nav-item ${isActive('/dispatch/customs-profiles') ? 'active' : ''}`}>
                <span className="nav-icon">🌍</span><span>Customs Profiles</span>
              </Link>
            </>)}
          </div>
          )}

          {/* ── INVENTORY (My Inventory always, WMS items gated) ──────── */}
          {/* FBA Inbound sits here as a specialised warehouse transfer */}
          <div className="nav-section">
            <button className="nav-section-title nav-section-toggle" onClick={() => toggleSection('inventory')}>
              <span>INVENTORY</span>
              <span className="nav-toggle-arrow">{expanded['inventory'] ? '▾' : '›'}</span>
            </button>
            {expanded['inventory'] && (<>
              ...
              {modules.wms && (<>
                ...
                <Link to="/stock/moves" className={`nav-item ${isActive('/stock/moves') ? 'active' : ''}`}>
                  <span className="nav-icon">🔀</span><span>Stock Moves</span>
                </Link>
              </>)}
              {modules.purchase_orders && (<>
          ...
          {/* ── RETURNS (module-gated) ────────────────────────────────── */}
          {modules.rma && (
          <div className="nav-section">
            <button className="nav-section-title nav-section-toggle" onClick={() => toggleSection('returns')}>
              <span>RETURNS</span>
              <span className="nav-toggle-arrow">{expanded['returns'] ? '▾' : '›'}</span>
            </button>
            {expanded['returns'] && (<>
          ...
          {/* ── AUTOMATION (module-gated) ──────────────────────────────── */}
          {modules.automation && (
          <div className="nav-section">
            <button className="nav-section-title nav-section-toggle" onClick={() => toggleSection('automation')}>
              <span>AUTOMATION</span>
              <span className="nav-toggle-arrow">{expanded['automation'] ? '▾' : '›'}</span>
            </button>
            {expanded['automation'] && (<>
              ...
              <Link to="/workflow-simulator" className={`nav-item ${isActive('/workflow-simulator') ? 'active' : ''}`}>
                <span className="nav-icon">🧪</span><span>Workflow Simulator</span>
              </Link>
          ...
          {/* ── ANALYTICS (basic always, advanced gated) ──────────────── */}
          <div className="nav-section">
            <button className="nav-section-title nav-section-toggle" onClick={() => toggleSection('analytics')}>
              <span>ANALYTICS</span>
              <span className="nav-toggle-arrow">{expanded['analytics'] ? '▾' : '›'}</span>
            </button>
            {expanded['analytics'] && (<>
              ...
              {modules.advanced_analytics && (<>
                ...
                <Link to="/analytics/pivot" className={`nav-item ${isActive('/analytics/pivot') ? 'active' : ''}`}>
                  <span className="nav-icon">🔀</span><span>Pivot Analytics</span>
                </Link>
                <Link to="/analytics/reporting" className={`nav-item ${isActive('/analytics/reporting') ? 'active' : ''}`}>
                  <span className="nav-icon">📊</span><span>Reporting Hub</span>
                </Link>
          ...
          {/* ── SETTINGS (always visible, single link) ────────────────── */}
          <div className="nav-section">
          ...
          {/* ── SUPPORT — Dev Tools flyout (unchanged) ────────────────── */}
            ...
            <div style={{ position: 'relative' }}>
              ...
              {supportOpen && (
                <div style={{
                  ...
                  <Link to="/dev/seed" className={`nav-item ${isActive('/dev/seed') ? 'active' : ''}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', padding: '6px 8px', borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>🌱</span><span>Data Seeder</span>
                  </Link>
                  <Link to="/ops" className={`nav-item ${isActive('/ops') ? 'active' : ''}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', padding: '6px 8px', borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>🖥️</span><span>Ops Console</span>
                  </Link>
                  <Link to="/dev/copy-products" className={`nav-item ${isActive('/dev/copy-products') ? 'active' : ''}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', padding: '6px 8px', borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>📋</span><span>Product Copy Tool</span>
                  </Link>
                  <Link to="/dev/jobs" className={`nav-item ${isActive('/dev/jobs') ? 'active' : ''}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', padding: '6px 8px', borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>⚙️</span><span>Job Monitor</span>
                  </Link>
                  <button onClick={() => setAmazonOpen(!amazonOpen)} className="nav-item"
                    style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', font: 'inherit', display: 'flex', alignItems: 'center', padding: '6px 8px', borderRadius: 6, fontSize: 13 }}>
                  ...
                  {amazonOpen && (
                    <div style={{ marginLeft: 12, marginTop: 2 }}>
                      <button onClick={navigateToSchemaManager}
                        className={`nav-item ${isActivePrefix('/marketplace/amazon/schemas') ? 'active' : ''}`}
                        style={{ width: '100%', textAlign: 'left', background: 'none', border: 'none', cursor: 'pointer', color: 'inherit', font: 'inherit', display: 'flex', alignItems: 'center', padding: '5px 8px', borderRadius: 6, fontSize: 12 }}>
                        <span style={{ marginRight: 6, fontSize: 12 }}>📋</span><span>Schema Manager</span>
                      </button>
                    </div>
                  )}
                  <Link to="/admin/schema-cache" className={`nav-item ${isActive('/admin/schema-cache') ? 'active' : ''}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', padding: '6px 8px', borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>🗄️</span><span>Schema Cache</span>
                  </Link>
                  <Link to="/dev/typesense" className={`nav-item ${isActive('/dev/typesense') ? 'active' : ''}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', padding: '6px 8px', borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>🔍</span><span>Typesense</span>
                  </Link>
                  <Link to="/dev/evri" className={`nav-item ${isActive("/dev/evri") ? "active" : ""}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: "none", display: "flex", alignItems: "center", padding: "6px 8px", borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>🟢</span><span>Evri API Tester</span>
                  </Link>
                  <Link to="/dev/royal-mail" className={`nav-item ${isActive("/dev/royal-mail") ? "active" : ""}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: "none", display: "flex", alignItems: "center", padding: "6px 8px", borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>📮</span><span>Royal Mail Tester</span>
                  </Link>
                  <Link to="/dev/oauth-onboarding" className={`nav-item ${isActive("/dev/oauth-onboarding") ? "active" : ""}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: "none", display: "flex", alignItems: "center", padding: "6px 8px", borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>🔗</span><span>OAuth Onboarding</span>
                  </Link>
                  <Link to="/dev/orders/seed" className={`nav-item ${isActive("/dev/orders/seed") ? "active" : ""}`}
                    onClick={() => { setSupportOpen(false); setAmazonOpen(false); }}
                    style={{ textDecoration: "none", display: "flex", alignItems: "center", padding: "6px 8px", borderRadius: 6, fontSize: 13 }}>
                    <span style={{ marginRight: 8, fontSize: 14 }}>🧪</span><span>Order Seeder</span>
                  </Link>
                </div>
              )}
            </div>
