...
export default function OrderActionsMenu({
  ...
  const menuItems: MenuItem[] = [
    ...
    },
    {
      ...
      children: [
        {
          type: 'action', label: 'Batch Assignment', icon: <Layers size={12} />,
          action: () => act(() => onBatchAssignment(effectiveIds)),
        },
        {
          type: 'action', label: 'Auto-assign Batches', icon: <Zap size={12} />,
          action: () => act(() => onAutoAssignBatches(effectiveIds)),
        },
        {
          type: 'action', label: 'Clear Batches', icon: <Trash2 size={12} />,
          action: () => act(() => onClearBatches(effectiveIds)),
        },
        {
          type: 'action', label: 'Link Unlinked Items', icon: <Link size={12} />,
        ...
        { type: 'separator' },
        {
          ...
          children: [
            {
              type: 'action', label: 'All Items', icon: <ClipboardList size={12} />,
              action: () => act(() => onAddItemsToPO(effectiveIds, 'all')),
            },
            {
              type: 'action', label: 'Out of Stock Only', icon: <Ban size={12} />,
