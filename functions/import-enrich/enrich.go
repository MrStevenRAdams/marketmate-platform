...
func processEnrichBatch(ctx context.Context, payload EnrichPayload) error {
	...
	if err != nil {
		// Credential not found — fall back to any active Amazon credential for this tenant
		log.Printf("[Enrich] Credential %s not found, scanning for active Amazon credential in tenant %s...", payload.CredentialID, payload.TenantID)
		creds, err = getAnyActiveAmazonCredential(ctx, fsClient, payload.TenantID)
		if err != nil {
	...
	for idx, item := range payload.Items {
		...
		// ── Keyword enrichment (non-blocking, best-effort) ────────────────────
		// Fires catalog vocabulary extraction and Amazon Ads recommendations
		// asynchronously via the main backend's internal endpoint.
		// DataForSEO is intentionally NOT triggered here — it fires only from
		// GenerateForAllChannels() or the manual keyword refresh endpoint.
		go callKeywordEnrich(context.Background(), item.ASIN, payload.TenantID, catalog2022)
...
// getAnyActiveAmazonCredential finds the first active Amazon credential across ALL tenants.
// Used as a fallback when the credential specified in the task no longer exists.
// In normal operation credentials are round-robined across all active Amazon accounts
// regardless of which tenant they belong to.
func getAnyActiveAmazonCredential(ctx context.Context, client *firestore.Client, preferTenantID string) (map[string]string, error) {
	// Scan all tenants — try the preferred tenant first for locality
	...
	for {
		...
		tid := doc.Ref.ID
		if tid == preferTenantID {
			allTenants = append([]string{tid}, allTenants...) // preferred tenant first
		} else {
	...
	for _, tid := range allTenants {
		iter := client.Collection("tenants").Doc(tid).
		...
		data := doc.Data()
		credID, _ := data["credential_id"].(string)
		log.Printf("[Enrich] Fallback: using active Amazon credential %s (tenant %s) instead of missing credential", credID, tid)
		return getDecryptedCredentials(ctx, client, tid, credID)
	}
	return nil, fmt.Errorf("no active Amazon credential found across any tenant")
}
...
// ── Keyword Enrichment (Session 2) ───────────────────────────────────────────

// callKeywordEnrich posts the raw GetCatalogItem response to the main backend's
// internal keyword enrichment endpoint. Always called in a goroutine — a failure
// here must never affect the enrichment job result.
//
// The main backend runs EnrichFromCatalogData + RefreshFromAmazonAdsAPI in its
// own goroutine and returns 202 immediately, so this call completes in <200ms.
// An OIDC identity token is fetched from the GCE metadata server so the request
// passes the backend's Cloud Run authentication.
func callKeywordEnrich(ctx context.Context, asin, tenantID string, catalogData map[string]interface{}) {
	backendURL := os.Getenv("API_BASE_URL")
	if backendURL == "" {
		backendURL = "https://marketmate-api-487246736287.europe-west2.run.app"
	}
	endpoint := backendURL + "/internal/keyword-intelligence/enrich"
	...
	if err != nil {
		log.Printf("[kw-enrich] marshal failed asin=%s: %v", asin, err)
		return
	}

	// Fetch OIDC identity token from the GCE metadata server.
	// The audience must be the base service URL (no path component).
	tokenURL := "http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/identity?audience=" + backendURL
	tokenReq, err := http.NewRequestWithContext(ctx, "GET", tokenURL, nil)
	if err != nil {
		log.Printf("[kw-enrich] build token request failed asin=%s: %v", asin, err)
		return
	}
	...
	if err != nil {
		log.Printf("[kw-enrich] OIDC token fetch failed asin=%s: %v", asin, err)
		return
	}
	...
	if idToken == "" {
		log.Printf("[kw-enrich] empty OIDC token asin=%s — skipping enrichment", asin)
		return
	}

	req, err := http.NewRequestWithContext(ctx, "POST", endpoint, bytes.NewReader(body))
	if err != nil {
		log.Printf("[kw-enrich] build request failed asin=%s: %v", asin, err)
		return
	}
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		log.Printf("[kw-enrich] request failed asin=%s: %v", asin, err)
		return
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusAccepted {
		...
		log.Printf("[kw-enrich] unexpected status %d asin=%s: %s", resp.StatusCode, asin, string(b))
	}
}
