...
// UpdateCredentialField updates arbitrary top-level fields on a credential document.
func (r *MarketplaceRepository) UpdateCredentialField(ctx context.Context, tenantID, credentialID string, fields map[string]interface{}) error {
    docRef := r.client.Collection("tenants").Doc(tenantID).
        Collection("marketplace_credentials").Doc(credentialID)
...
// FindListingByChannelAndSKU checks whether an active listing already exists for a
// given channel + SKU combination. Used during import to skip pending_review for
// products that have already been confirmed and listed on this channel.
// Only matches non-ended listings — ended listings allow re-import.
func (r *MarketplaceRepository) FindListingByChannelAndSKU(ctx context.Context, tenantID, channel, sku string) (*models.Listing, error) {
	iter := r.client.Collection("tenants").Doc(tenantID).
		Collection("listings").
		Where("channel", "==", channel).
		Where("channel_identifiers.sku", "==", sku).
		Limit(1).
		Documents(ctx)
	defer iter.Stop()
	...
	// Don't treat ended listings as blocking — the product was removed and can be re-imported
...
// FindListingByExternalID looks up a listing by channel + external marketplace ID
// (e.g. Temu goodsId stored in channel_identifiers.listing_id).
// This is the correct key for reconciliation — NOT the SKU, which can be reused.
func (r *MarketplaceRepository) FindListingByExternalID(ctx context.Context, tenantID, channel, externalID string) (*models.Listing, error) {
	iter := r.client.Collection("tenants").Doc(tenantID).
		Collection("listings").
		Where("channel", "==", channel).
		Where("channel_identifiers.listing_id", "==", externalID).
		Limit(1).
		Documents(ctx)
	defer iter.Stop()
...
// ListListingsByAccount returns all listings for a specific channel account.
// Used by reconciliation to find stale listings after an import.
// ============================================================================
// IMPORT JOBS
// ============================================================================
...
func extractImportJob(jobID, tenantID string, data map[string]interface{}) models.ImportJob {
	...
	if v, ok := data["account_name"].(string);        ok { job.AccountName = v }
	if v, ok := data["status_message"].(string);      ok { job.StatusMessage = v }
	if v, ok := data["enrich_data"].(bool);           ok { job.EnrichData = v }
	if v, ok := data["pending_review"].(bool);        ok { job.PendingReview = v }
	if v, ok := data["sync_stock"].(bool);            ok { job.SyncStock = v }
	if v, ok := data["ai_optimize"].(bool);           ok { job.AIOptimize = v }
	if v, ok := data["fulfillment_filter"].(string);  ok { job.FulfillmentFilter = v }
	if v, ok := data["stock_filter"].(string);        ok { job.StockFilter = v }
...
func (r *MarketplaceRepository) SaveExtendedData(ctx context.Context, tenantID string, data *models.ExtendedProductData) error {
	return r.SaveExtendedDataToCollection(ctx, tenantID, "products", data)
}

// SaveExtendedDataToCollection saves extended data to any product-shaped collection
// (either "products" or "pending_imports"). Used to support enrichment during review.
func (r *MarketplaceRepository) SaveExtendedDataToCollection(ctx context.Context, tenantID, collection string, data *models.ExtendedProductData) error {
	if collection == "" {
		collection = "products"
	}
	docRef := r.client.Collection("tenants").Doc(tenantID).
		Collection(collection).Doc(data.ProductID).
...
// PENDING IMPORTS
// ============================================================================
// Stored as: tenants/{tenant}/pending_imports/{pending_import_id}
// Products sit here while awaiting user review. They are NOT in the products
// collection until the user confirms them. On confirmation they are moved to
// products/ and the mapping source_collection is updated accordingly.
// ============================================================================

// CreatePendingImport writes a product-shaped document to pending_imports.
func (r *MarketplaceRepository) CreatePendingImport(ctx context.Context, tenantID string, doc map[string]interface{}) error {
	id, _ := doc["product_id"].(string)
	if id == "" {
		return fmt.Errorf("pending import doc must have product_id")
	}
	_, err := r.client.Collection("tenants").Doc(tenantID).
		Collection("pending_imports").Doc(id).Set(ctx, doc)
	return err
}

// GetPendingImport fetches a single pending import by its ID.
func (r *MarketplaceRepository) GetPendingImport(ctx context.Context, tenantID, pendingID string) (map[string]interface{}, error) {
	doc, err := r.client.Collection("tenants").Doc(tenantID).
		Collection("pending_imports").Doc(pendingID).Get(ctx)
	if err != nil {
		return nil, err
	}
	return doc.Data(), nil
...
// ListPendingImportsByJob returns all pending imports for a specific import job.
func (r *MarketplaceRepository) ListPendingImportsByJob(ctx context.Context, tenantID, jobID string) ([]map[string]interface{}, error) {
	iter := r.client.Collection("tenants").Doc(tenantID).
...
// MovePendingToProducts moves a pending_imports doc into the products collection
// and deletes it from pending_imports. Called when user confirms import as new.
// Returns the product_id that was moved.
func (r *MarketplaceRepository) MovePendingToProducts(ctx context.Context, tenantID, pendingID string, extraUpdates map[string]interface{}) (string, error) {
	pendingRef := r.client.Collection("tenants").Doc(tenantID).
		Collection("pending_imports").Doc(pendingID)
	
	snap, err := pendingRef.Get(ctx)
	if err != nil {
		return "", fmt.Errorf("pending import not found: %w", err)
	}
	data := snap.Data()

	// Apply any extra updates (e.g. status: "active")
	...
	if productID == "" {
		return "", fmt.Errorf("pending import has no product_id")
	}

	// Write to products collection
	_, err = r.client.Collection("tenants").Doc(tenantID).
		Collection("products").Doc(productID).Set(ctx, data)
	if err != nil {
		return "", fmt.Errorf("failed to create product: %w", err)
	}

	// Delete from pending_imports
	_, err = pendingRef.Delete(ctx)
	if err != nil {
		// Non-fatal — product was created, log and continue
		log.Printf("[Repo] MovePendingToProducts: failed to delete pending doc %s: %v", pendingID, err)
	}

	return productID, nil
}

// DeletePendingImport removes a pending import doc. Called on reject.
func (r *MarketplaceRepository) DeletePendingImport(ctx context.Context, tenantID, pendingID string) error {
	_, err := r.client.Collection("tenants").Doc(tenantID).
		Collection("pending_imports").Doc(pendingID).Delete(ctx)
	return err
}

// UpdatePendingImport updates fields on a pending import doc (e.g. enrichment_status).
func (r *MarketplaceRepository) UpdatePendingImport(ctx context.Context, tenantID, pendingID string, updates map[string]interface{}) error {
	var fsUpdates []firestore.Update
	for k, v := range updates {
		fsUpdates = append(fsUpdates, firestore.Update{Path: k, Value: v})
	}
	_, err := r.client.Collection("tenants").Doc(tenantID).
		Collection("pending_imports").Doc(pendingID).Update(ctx, fsUpdates)
	return err
}

// CopyPendingExtendedDataToProduct copies extended_data subcollection docs
// from a pending import to an existing confirmed product. Used when merging
// a pending product into an existing one on Accept.
func (r *MarketplaceRepository) CopyPendingExtendedDataToProduct(ctx context.Context, tenantID, pendingID, targetProductID string) error {
	srcCol := r.client.Collection("tenants").Doc(tenantID).
		Collection("pending_imports").Doc(pendingID).Collection("extended_data")
	dstCol := r.client.Collection("tenants").Doc(tenantID).
		Collection("products").Doc(targetProductID).Collection("extended_data")

	iter := srcCol.Documents(ctx)
	defer iter.Stop()
	for {
		doc, err := iter.Next()
		if err == iterator.Done {
		...
		_, err = dstCol.Doc(doc.Ref.ID).Set(ctx, doc.Data())
		if err != nil {
			log.Printf("[Repo] CopyPendingExtendedData: failed to copy %s: %v", doc.Ref.ID, err)
		}
	}
	return nil
}

// PendingImportCount returns the number of pending imports for a tenant.
// Used by the review badge count endpoint.
func (r *MarketplaceRepository) PendingImportCount(ctx context.Context, tenantID string) (int64, error) {
	result, err := r.client.Collection("tenants").Doc(tenantID).
		Collection("pending_imports").
