...
type AIConsolidationHandler struct {
	...
	kwSvc            *services.KeywordIntelligenceService
}
...
func (h *AIConsolidationHandler) generateListingsForProduct(
	...
	// Build keyword cache key from ASIN if available, otherwise product ID.
	cacheKey := productID
	if asin != "" {
		cacheKey = asin
	}

	// Fetch the KeywordSet once — shared raw data, adapted per channel.
	// GetOrCreateKeywordSet is nil-safe on error: generation continues without
	// keyword context if the cache miss or refresh fails.
	...
	if len(channels) == 1 {
		// Single channel: straightforward adapter transform.
		kwCtx := keyword.Get(channels[0]).Transform(ks) // nil-safe: Transform(nil) returns nil
		return h.listingGenSvc.GenerateForAllChannels(ctx, req, kwCtx)
	}

	// Multi-channel: each channel gets its own KeywordContext derived from the
	// same KeywordSet but adapted for channel-specific constraints.
	var allListings []services.AIListingOutput
	for _, channel := range channels {
		kwCtx := keyword.Get(channel).Transform(ks) // per-channel adaptation
		singleReq := req
		singleReq.Channels = []string{channel}
		result, chanErr := h.listingGenSvc.GenerateForAllChannels(ctx, singleReq, kwCtx)
		if chanErr != nil {
			log.Printf("[AI] channel %s generation failed: %v", channel, chanErr)
			allListings = append(allListings, services.AIListingOutput{
				Channel:  channel,
				Warnings: []string{fmt.Sprintf("Generation failed: %v", chanErr)},
			})
			continue
		}
		if result != nil {
	...
	combined := &services.MultiChannelListingResult{
		ProductID: productID,
