...
type OrderHandler struct {
	...
	// Shopline
	shoplineHandler         *ShoplineHandler
}
...
func (h *OrderHandler) SetShoplineHandler(handler *ShoplineHandler) {
	h.shoplineHandler = handler
}
...
func (h *OrderHandler) processChannelImport(tenantID, jobID, channel, channelAccountID, dateFrom, dateTo string) {
	...
	case "shopline":
		if h.shoplineHandler != nil {
			imported, importErr = h.shoplineHandler.ImportShoplineOrders(ctx, tenantID, channelAccountID, createdAfter, createdBefore)
		} else {
			importErr = fmt.Errorf("Shopline handler not initialised")
		}
	case "shopwired":
