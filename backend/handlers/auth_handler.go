...
func (h *AuthHandler) InviteUser(c *gin.Context) {
	...
	go func() {
		subject := "You've been invited to MarketMate"
...
</body></html>`, tenantID, req.Role, inviteLink)

		if emailErr := services.SendRawEmail(req.Email, subject, htmlBody); emailErr != nil {
			log.Printf("[auth] failed to send invitation email to %s: %v", req.Email, emailErr)
		} else {
			log.Printf("[auth] invitation email sent to %s", req.Email)
		}
	}()
