...
func (h *SettingsHandler) GetNotificationSettings(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			...
			"cancellation_policy": defaultCancellationPolicyMap(),
		})
		return
	}
	data := doc.Data()
	...
	policy := GetCancellationPolicy(ctx, h.client, tenantID)
	c.JSON(http.StatusOK, gin.H{
...
func defaultCancellationPolicyMap() map[string]interface{} {
...
func (h *SettingsHandler) UpdateNotificationSettings(c *gin.Context) {
	...
	if req.CancellationPolicy != nil {
		if err := SaveCancellationPolicy(ctx, h.client, tenantID, *req.CancellationPolicy); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to save cancellation policy: " + err.Error()})
			return
		}
	}
	c.JSON(http.StatusOK, gin.H{"ok": true})
...
func (h *SettingsHandler) GetModules(c *gin.Context) {
	...
	if err != nil {
		// Default: MVP defaults — advanced features hidden until explicitly enabled per-tenant
		c.JSON(200, gin.H{"modules": EnabledModules{
			WMS: false, AdvancedDispatch: false, PurchaseOrders: false,
			Automation: true, RMA: false, AdvancedAnalytics: false, EmailSystem: true,
		}})
		return
	}
