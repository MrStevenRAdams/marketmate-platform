...
func (h *EbayEnrichmentHandler) BulkEnrich(c *gin.Context) {
	...
	var req struct {
		CredentialID string `json:"credential_id"` // optional — auto-resolved if empty
...
//
// If credentialID is provided it must belong to tenantID.
// If empty, the handler first checks the tenant's own eBay credentials, then
// falls back to any active production eBay credential across ALL tenants.
// This allows enrichment to work even when the requesting tenant has no eBay
// account of their own — the eBay Browse API returns public product catalogue
// data only, so no seller-specific data is exposed.
func (h *EbayEnrichmentHandler) buildEbayClient(ctx context.Context, tenantID, credentialID string) (*ebayClient.Client, error) {
	buildFromCred := func(cred *models.MarketplaceCredential) (*ebayClient.Client, error) {
		if cred.Channel != "ebay" && cred.Channel != "ebay_sandbox" {
			return nil, fmt.Errorf("credential %s is not an eBay credential", cred.CredentialID)
		}
		...
		if refresh := cred.CredentialData["refresh_token"]; refresh != "" {
			...
			return nil, fmt.Errorf("no OAuth token found in credential %s", cred.CredentialID)
		}
	...
	// 1. Explicit credential ID from this tenant
	if credentialID != "" {
	...
	// 2. Any active eBay credential on this tenant
	creds, err := h.repo.ListCredentials(ctx, tenantID)
	if err == nil {
		for _, c := range creds {
			if (c.Channel == "ebay") && c.Active {
				cp := c
				if client, err := buildFromCred(&cp); err == nil {
					log.Printf("[EbayEnrich] Using own eBay credential %s for tenant %s", cp.CredentialID, tenantID)
					return client, nil
				}
			}
		}
	...
	// 3. Cross-tenant fallback — find any active production eBay credential
	log.Printf("[EbayEnrich] No eBay credential on tenant %s — searching all tenants", tenantID)
	iter := h.fsClient.CollectionGroup("marketplace_credentials").
		Where("channel", "==", "ebay").
		Where("active", "==", true).
	...
	for {
		...
		client, err := buildFromCred(&cred)
		if err != nil {
			log.Printf("[EbayEnrich] Cross-tenant credential %s from tenant %s failed: %v", cred.CredentialID, cred.TenantID, err)
			continue
		}
		log.Printf("[EbayEnrich] Using cross-tenant eBay credential %s from tenant %s", cred.CredentialID, cred.TenantID)
		return client, nil
	}

	return nil, fmt.Errorf("no active eBay credential found on any tenant")
}
