...
func (h *TemuHandler) PrepareTemuListing(c *gin.Context) {
	...
	if err == nil && temuExtData != nil {
		...
		// DIAGNOSTIC: log what we found in extended_data
		extKeys := make([]string, 0, len(temuExtData))
		for k := range temuExtData {
			extKeys = append(extKeys, k)
		}
		rawKeys := make([]string, 0)
		if temuRawData != nil {
			for k := range temuRawData {
				rawKeys = append(rawKeys, k)
			}
		}
		log.Printf("[Temu Prepare DIAGNOSTIC] product=%s extData_keys=%v raw_keys=%v hasImages=%v hasGallery=%v",
			req.ProductID, extKeys, rawKeys,
			temuRawData["carouselImage"] != nil || temuRawData["mainImageUrl"] != nil,
			temuRawData["goodsGallery"] != nil)
	} else {
		log.Printf("[Temu Prepare DIAGNOSTIC] product=%s NO extended_data (err=%v)", req.ProductID, err)
	}
	...
	if temuRawData != nil {
		...
		// Images — check all known field name variants
		if len(draft.Images) == 0 {
			extractImgStrings := func(val interface{}) []string {
				var out []string
				if arr, ok := val.([]interface{}); ok {
					for _, item := range arr {
						if s, ok := item.(string); ok && s != "" {
							...
							for _, uk := range []string{"url", "imageUrl", "image_url"} {
								if s, ok := m[uk].(string); ok && s != "" {
									out = append(out, s)
									break
								}
							}
			...
			// Try top-level carousel/images keys
			for _, key := range []string{"carouselImage", "carousel_image", "images"} {
				if imgs := extractImgStrings(temuRawData[key]); len(imgs) > 0 {
					draft.Images = imgs
					break
				}
			}

			// Try goodsGallery wrapper
			for _, galleryKey := range []string{"goodsGallery", "gallery"} {
				if len(draft.Images) > 0 {
					break
				}
				if gallery, ok := temuRawData[galleryKey].(map[string]interface{}); ok {
					// Main image first
					var galleryImgs []string
					for _, key := range []string{"mainImage", "mainImageUrl", "main_image"} {
						if s, ok := gallery[key].(string); ok && s != "" {
							galleryImgs = append(galleryImgs, s)
							break
						}
					}
					// Then carousel
					for _, key := range []string{"carouselImage", "carousel_image", "images"} {
						if imgs := extractImgStrings(gallery[key]); len(imgs) > 0 {
							for _, img := range imgs {
								if img != "" && (len(galleryImgs) == 0 || galleryImgs[0] != img) {
									galleryImgs = append(galleryImgs, img)
								}
							}
							break
						}
					...
					for _, key := range []string{"detailImage", "detail_image", "detailImages"} {
						if imgs := extractImgStrings(gallery[key]); len(imgs) > 0 {
							galleryImgs = append(galleryImgs, imgs...)
							break
						}
					}
					if len(galleryImgs) > 0 {
						draft.Images = galleryImgs
					}
				}
			}

			// Top-level detailImage fallback
			if len(draft.Images) == 0 {
				for _, key := range []string{"detailImage", "detail_image", "detailImages"} {
					if imgs := extractImgStrings(temuRawData[key]); len(imgs) > 0 {
						draft.Images = imgs
...
func (h *TemuHandler) SubmitTemuListing(c *gin.Context) {
	...
	// ── Session 8 fix: Temu error 150010072 ─────────────────────────────────
	// resolveSpecIdsFromTemplate returns one specId per parentSpec dimension
	// (e.g. Colour + Size = 2 specIds). For a simple non-variant product the
	// skuList entry must contain exactly ONE specId — sending more triggers
	// "Number of specifications exceeds limit" (code 150010072).
	// For variant products the per-variant spec resolution overrides this list,
	// so truncating here is also safe.
	if len(specIdList) > 1 {
		log.Printf("[Temu] Truncating specIdList from %d to 1 for simple-product SKU (was: %v)", len(specIdList), specIdList)
		specIdList = specIdList[:1]
	}
	...
	if isUpdate && req.CredentialID != "" {
		...
		if existingListing != nil {
			...
			existingListing.Overrides = buildTemuListingOverrides(req, temuImages)
		...
		listing := &models.Listing{
			...
			Overrides: buildTemuListingOverrides(req, temuImages),
			CreatedAt: now,
			UpdatedAt: now,
		}
...
func buildTemuDraft(
	...
	// The PIM stores price as an integer pence value (e.g. 750 for £7.50).
	// We divide by 100 here so the frontend receives a proper currency string
	// (e.g. "7.50") and can submit it directly to Temu without further conversion.
	if price, ok := product["price"].(float64); ok && price > 0 {
		draft.Price = &TemuPrice{
			BaseAmount: fmt.Sprintf("%.2f", price/100),
			Currency:   "GBP",
		}
	}
...
// buildTemuListingOverrides builds a complete ListingOverrides from a Temu
// submit request. Storing the full listing data here means that when the user
// edits the listing later we can restore everything from Firestore without
// needing to re-run the AI prepare step.
func buildTemuListingOverrides(req TemuSubmitRequest, images []string) *models.ListingOverrides {
	price, _ := strconv.ParseFloat(req.Price.BaseAmount, 64)
	qty := req.Quantity

	// Pack Temu-specific fields that don't have first-class columns into Attributes
	attrs := map[string]interface{}{
		"shippingTemplate": req.ShippingTemplate,
		"fulfillmentType":  req.FulfillmentType,
		"shipmentLimitDay": req.PrepDays,
	}
	if req.OriginInfo != nil {
		if r1, ok := req.OriginInfo["originRegion1"].(string); ok {
			attrs["originRegion1"] = r1
		}
		if r2, ok := req.OriginInfo["originRegion2"].(string); ok {
			attrs["originRegion2"] = r2
		}
	}
	if len(req.GoodsProperties) > 0 {
		attrs["goodsProperties"] = req.GoodsProperties
	}
	if req.Brand != nil {
		attrs["brand"] = req.Brand
	}

	return &models.ListingOverrides{
		Title:           req.Title,
