...
// ============================================================================
// SHIPPING RULES — GET/POST/PUT/DELETE /api/v1/dispatch/shipping-rules
// ============================================================================
// Auto-assign a carrier and service when an order matches conditions.
// Rules are evaluated in priority order (lowest number = highest priority).

type ShippingRuleCondition struct {
	Field    string `json:"field" firestore:"field"`       // channel, order_value_gte, order_value_lt, weight_gte_kg, weight_lt_kg, destination_country, sku_prefix
	Operator string `json:"operator" firestore:"operator"` // eq, gte, lt, contains
	Value    string `json:"value" firestore:"value"`
}

type ShippingRuleAction struct {
	CarrierID    string `json:"carrier_id" firestore:"carrier_id"`
	ServiceCode  string `json:"service_code" firestore:"service_code"`
	ServiceLabel string `json:"service_label,omitempty" firestore:"service_label,omitempty"`
}
...
type ShippingRule struct {
	...
	Priority   int                     `json:"priority" firestore:"priority"`
	IsActive   bool                    `json:"is_active" firestore:"is_active"`
	Conditions []ShippingRuleCondition `json:"conditions" firestore:"conditions"`
	Actions    ShippingRuleAction      `json:"actions" firestore:"actions"`
	CreatedAt  time.Time               `json:"created_at" firestore:"created_at"`
	UpdatedAt  time.Time               `json:"updated_at" firestore:"updated_at"`
}

func (h *DispatchExtensionsHandler) shippingRulesCol(tenantID string) *firestore.CollectionRef {
	return h.client.Collection("tenants").Doc(tenantID).Collection("shipping_rules")
}

func (h *DispatchExtensionsHandler) ListShippingRules(c *gin.Context) {
	tenantID := h.tenantID(c)
	ctx := c.Request.Context()

	iter := h.shippingRulesCol(tenantID).OrderBy("priority", firestore.Asc).Documents(ctx)
	defer iter.Stop()
...
func (h *DispatchExtensionsHandler) CreateShippingRule(c *gin.Context) {
	tenantID := h.tenantID(c)
	...
	if rule.Conditions == nil {
		rule.Conditions = []ShippingRuleCondition{}
	}

	_, err := h.shippingRulesCol(tenantID).Doc(rule.RuleID).Set(c.Request.Context(), rule)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to create rule"})
		return
	}
...
func (h *DispatchExtensionsHandler) UpdateShippingRule(c *gin.Context) {
	tenantID := h.tenantID(c)
	ruleID := c.Param("rule_id")
	var rule ShippingRule
	if err := c.ShouldBindJSON(&rule); err != nil {
	...
	_, err := h.shippingRulesCol(tenantID).Doc(ruleID).Set(c.Request.Context(), rule, firestore.MergeAll)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to update rule"})
		return
	}
...
func (h *DispatchExtensionsHandler) DeleteShippingRule(c *gin.Context) {
	tenantID := h.tenantID(c)
	ruleID := c.Param("rule_id")

	_, err := h.shippingRulesCol(tenantID).Doc(ruleID).Delete(c.Request.Context())
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "failed to delete rule"})
		return
	}
