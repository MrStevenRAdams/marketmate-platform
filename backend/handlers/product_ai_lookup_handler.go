...
// getEbayClient resolves an eBay credential and returns an authenticated client.
//
// Resolution order:
//  1. If credentialID is provided, use that credential from the requesting tenant.
//  2. If the requesting tenant has an active eBay credential, use it.
//  3. Search ALL tenants for any active eBay credential (same locale preferred).
//     This allows EAN lookups to work even when the requesting tenant has no
...
func (h *ProductAILookupHandler) getEbayClient(ctx context.Context, tenantID, credentialID string) (*ebay.Client, error) {
	// Helper: build client from a credential record
	buildClient := func(cred *models.MarketplaceCredential) (*ebay.Client, error) {
	...
	// 1. Explicit credential ID — use the requesting tenant's credential directly
	if credentialID != "" {
		cred, err := h.marketplaceRepo.GetCredential(ctx, tenantID, credentialID)
		if err != nil {
			return nil, err
		}
		return buildClient(cred)
	}

	// 2. Check the requesting tenant's own eBay credentials first
	creds, err := h.marketplaceRepo.ListCredentials(ctx, tenantID)
	if err == nil {
		for _, c := range creds {
			if (c.Channel == "ebay" || c.Channel == "ebay_sandbox") && c.Active {
				cp := c
				client, err := buildClient(&cp)
				if err == nil {
					log.Printf("[AILookup] Using eBay credential %s from requesting tenant %s", cp.CredentialID, tenantID)
					return client, nil
				}
			}
		}
	...
	// 3. No eBay connection on this tenant — search all tenants for any active eBay credential.
	// This is intentional: eBay Browse API is used purely for product data enrichment
	// (EAN → product title/image/EPID). No orders or seller-specific data are accessed.
	log.Printf("[AILookup] No eBay credential on tenant %s — searching all tenants for eBay connection", tenantID)
	fsClient := h.marketplaceRepo.GetFirestoreClient()
	if fsClient == nil {
		return nil, fmt.Errorf("no active eBay credential found and Firestore unavailable for cross-tenant search")
	}

	// Query marketplace_credentials across all tenants for active eBay credentials.
	// Prefer production over sandbox. We use a collection-group query.
	iter := fsClient.CollectionGroup("marketplace_credentials").
		Where("channel", "in", []string{"ebay"}).
		Where("active", "==", true).
		Limit(10).
		Documents(ctx)
	defer iter.Stop()
	...
	for {
		...
		// Skip the requesting tenant (already checked above)
		if cred.TenantID == tenantID {
			continue
		}
		// Skip sandbox credentials — use production eBay data for enrichment
		if cred.Environment == "sandbox" {
			continue
		}
		client, err := buildClient(&cred)
		if err != nil {
			log.Printf("[AILookup] Cross-tenant eBay credential %s from tenant %s failed: %v", cred.CredentialID, cred.TenantID, err)
			continue
		}
		log.Printf("[AILookup] Using cross-tenant eBay credential %s from tenant %s for EAN enrichment", cred.CredentialID, cred.TenantID)
		return client, nil
	}

	return nil, fmt.Errorf("no active eBay credential found on any tenant — connect an eBay account to enable EAN lookup")
}
