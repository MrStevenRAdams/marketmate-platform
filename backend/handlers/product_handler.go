...
func NewProductHandler(productService *services.ProductService, searchService *services.SearchService, client *firestore.Client) *ProductHandler {
...
func (h *ProductHandler) CreateProduct(c *gin.Context) {
	...
	// Auto-create inventory record for simple and variant products
	if product.ProductType == "simple" || product.ProductType == "variant" {
		go func() {
			bgCtx := context.Background()
			CreateInventoryRecord(bgCtx, h.client, tenantID, product.SKU, product.Title)
		}()
	}
...
// CreateInventoryRecord creates a zero-quantity inventory record for a SKU.
// Called on product creation and on import. Idempotent — skips if already exists.
func CreateInventoryRecord(ctx context.Context, client *firestore.Client, tenantID, sku, productName string) {
	if sku == "" {
		return
	}
	invID := "inv_" + strings.ReplaceAll(strings.ReplaceAll(sku, "/", "_"), " ", "_")
	ref := client.Collection("tenants").Doc(tenantID).Collection("inventory").Doc(invID)
	...
	sources := client.Collection("tenants").Doc(tenantID).Collection("fulfilment_sources").
		Where("active", "==", true).Documents(ctx)
	defer sources.Stop()
	for {
		snap, err := sources.Next()
		if err != nil {
			break
		}
		data := snap.Data()
		if data["default"] == true {
			warehouseID, _ = data["source_id"].(string)
			warehouseName, _ = data["name"].(string)
			break
		}
	...
	if err != nil {
		log.Printf("[inventory] failed to create record for SKU %s tenant %s: %v", sku, tenantID, err)
	}
}
