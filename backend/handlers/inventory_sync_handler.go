...
// Run starts two background goroutines:
//
//  1. Incremental sync — runs every 15 minutes, pushes stock for all enabled
//     credentials. This is the primary path triggered by stock movements.
//
//  2. Daily full reconciliation — runs at midnight UTC every day, ensures that
//     any updates missed by the incremental pass (e.g. due to transient errors,
//     deployments, or listing additions) are caught. The full pass iterates
//     every listing for every enabled credential regardless of change state.
func (s *InventorySyncScheduler) Run() {
	...
	go func() {
		...
		// Run once immediately after warm-up delay
		s.runAllTenants()
	...
	// ── Daily full reconciliation at midnight UTC ─────────────────────────────
	go func() {
		for {
			now := time.Now().UTC()
			// Calculate duration until next midnight UTC
			nextMidnight := time.Date(now.Year(), now.Month(), now.Day()+1, 0, 0, 0, 0, time.UTC)
			// Add a 2-minute offset to avoid racing with any midnight-triggered jobs
			nextRun := nextMidnight.Add(2 * time.Minute)
			waitDuration := nextRun.Sub(now)

			log.Printf("[InventorySync] Daily full reconciliation scheduled in %s (at %s UTC)",
				waitDuration.Round(time.Minute).String(),
			...
			log.Println("[InventorySync] Starting daily full reconciliation...")
			s.runAllTenants()
			log.Println("[InventorySync] Daily full reconciliation complete")
		}
	}()
}
