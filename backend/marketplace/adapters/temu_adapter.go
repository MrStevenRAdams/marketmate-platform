...
func (t *TemuAdapter) FetchListings(ctx context.Context, filters marketplace.ImportFilters) ([]marketplace.MarketplaceProduct, error) {
	...
	// Always include status=6 (Deleted) for reconciliation purposes — we need to
	// know which goodsIds have been deleted on Temu so we can mark those Firestore
	// listings as "ended". Deleted products are identified by MarketplaceStatus="deleted"
	// and the service handles them separately (no PIM create, just listing cleanup).
	hasDeleted := false
	...
	if !hasDeleted {
		statusFilters = append(statusFilters, 6)
	}
	...
	for _, statusFilter := range statusFilters {
		...
		for {
			...
			// Only signal on page 1 — Temu reports the same total on every page
			// and the service accumulates __total__ signals across list types.
			if page == 1 && listPage.Total > 0 && filters.ProgressCallback != nil {
			...
			for _, goods := range listPage.GoodsList {
				...
				// For deleted products skip the expensive GetGoodsDetail call —
			// we only need the goodsId for reconciliation, not full data.
			if statusFilter == 6 {
				product = t.convertListItemToProduct(&goods)
				product.MarketplaceStatus = "deleted"
			} else {
				...
				case 1:
					if goods.OnSale == 1 {
						product.MarketplaceStatus = "active"
					} else {
						product.MarketplaceStatus = "inactive"
					}
				case 4:
					product.MarketplaceStatus = "incomplete"
				case 5:
					product.MarketplaceStatus = "draft"
				}
			}
...
// SyncInventory pushes updated stock quantities to Temu for all SKUs of a product.
// externalID is the Temu goodsId stored in our listings collection.
// quantity is the total available stock across all SKUs. For multi-SKU products
// we distribute the total evenly across active SKUs; single-SKU products get
// the full quantity. Temu requires per-SKU stock rather than a product total.
func (t *TemuAdapter) SyncInventory(ctx context.Context, externalID string, quantity int) error {
	var goodsID int64
	if _, err := fmt.Sscanf(externalID, "%d", &goodsID); err != nil {
		return fmt.Errorf("temu sync inventory: invalid goodsId %q: %w", externalID, err)
	}

	// Fetch current SKU list so we know every skuId for this product.
	skus, err := t.client.ListSKUs(goodsID)
	if err != nil {
		return fmt.Errorf("temu sync inventory: list SKUs for goods %d: %w", goodsID, err)
	}
	if len(skus) == 0 {
		return fmt.Errorf("temu sync inventory: no SKUs found for goods %d", goodsID)
	}

	// Distribute quantity across SKUs.
	// Single-SKU products receive the full quantity.
	// Multi-SKU products receive an even split; any remainder goes to the first SKU.
	updates := make([]temu.SKUStockUpdate, len(skus))
	perSKU := quantity / len(skus)
	remainder := quantity % len(skus)
	for i, sku := range skus {
		...
		updates[i] = temu.SKUStockUpdate{SkuID: sku.SkuID, StockNum: qty}
	}

	if err := t.client.UpdateSKUStock(goodsID, updates); err != nil {
		return fmt.Errorf("temu sync inventory goods %d: %w", goodsID, err)
	}
	return nil
}

// SyncPrice pushes an updated retail price to Temu for all SKUs of a product.
// externalID is the Temu goodsId. price is in the seller account's configured currency.
// Temu stores prices as decimal strings; we format to 2dp.
func (t *TemuAdapter) SyncPrice(ctx context.Context, externalID string, price float64) error {
	var goodsID int64
	if _, err := fmt.Sscanf(externalID, "%d", &goodsID); err != nil {
		return fmt.Errorf("temu sync price: invalid goodsId %q: %w", externalID, err)
	}
	if price <= 0 {
		return fmt.Errorf("temu sync price: price must be positive, got %f", price)
	}

	// Fetch current SKUs to get their IDs and existing currency.
	skus, err := t.client.ListSKUs(goodsID)
	if err != nil {
		return fmt.Errorf("temu sync price: list SKUs for goods %d: %w", goodsID, err)
	}
	if len(skus) == 0 {
		return fmt.Errorf("temu sync price: no SKUs found for goods %d", goodsID)
	}

	// Determine currency from first SKU's price map; default to USD.
	currencyType := "USD"
	if len(skus) > 0 {
		if ct, ok := skus[0].Price["currencyType"].(string); ok && ct != "" {
			currencyType = ct
		} else if ct, ok := skus[0].Price["currency_type"].(string); ok && ct != "" {
			currencyType = ct
		}
	}

	priceStr := fmt.Sprintf("%.2f", price)
	updates := make([]temu.SKUPriceUpdate, len(skus))
	for i, sku := range skus {
		updates[i] = temu.SKUPriceUpdate{
			SkuID: sku.SkuID,
			Price: temu.SKUPriceValue{
	...
	if err := t.client.UpdateSKUPrice(goodsID, updates); err != nil {
		return fmt.Errorf("temu sync price goods %d: %w", goodsID, err)
	}
	return nil
}
