...
// SyncPrice updates the price of an eBay listing via the Inventory Offer API.
// externalID is the seller SKU (outSkuSn) stored in our listings collection.
// The method fetches the offer for this SKU, determines the marketplace currency,
// then patches the offer price. If no offer is found, the update is skipped
// (the listing may have been created via Trading API and will be picked up on
// the next full reconciliation).
func (e *EbayAdapter) SyncPrice(ctx context.Context, externalID string, price float64) error {
	if price <= 0 {
		return fmt.Errorf("ebay sync price: price must be positive, got %f", price)
	}

	// externalID may be either an eBay item ID or a seller SKU depending on
	// how the listing was created. Try the offer lookup by SKU first.
	offer, err := e.client.GetOfferBySKU(externalID)
	if err != nil {
		// GetOfferBySKU returns nil,nil when no offer exists — any other error is real
		return fmt.Errorf("ebay sync price: get offer for SKU %q: %w", externalID, err)
	}
	if offer == nil {
		// No Inventory API offer found — listing was likely created via Trading API.
		// Log and return without error; a future reconciliation pass will handle this.
		log.Printf("[eBay SyncPrice] no offer found for SKU %q — skipping (Trading API listing?)", externalID)
		return nil
	}

	// Determine currency from the existing offer's pricing summary.
	currency := "GBP"
	if offer.PricingSummary != nil && offer.PricingSummary.Price != nil {
		if offer.PricingSummary.Price.Currency != "" {
			currency = offer.PricingSummary.Price.Currency
		}
	}

	if err := e.client.UpdateOfferPrice(offer.OfferID, price, currency); err != nil {
		return fmt.Errorf("ebay sync price offer %s SKU %q: %w", offer.OfferID, externalID, err)
	}
	return nil
}
