...
type ImportJob struct {
	...
	// Review Mappings flow
	// PendingReview=true means this job was auto-triggered on channel connect.
	// Products are staged (status="pending_review") and the match analysis runs
	// automatically in the background. The user reviews via /products/review-mappings.
	PendingReview    bool                   `firestore:"pending_review,omitempty" json:"pending_review,omitempty"`
	MatchStatus      string                 `firestore:"match_status,omitempty" json:"match_status,omitempty"` // "analyzing"|"review_required"|"no_review_needed"|"reviewed"
...
type ImportMapping struct {
	...
	// SourceCollection indicates where the product currently lives.
	// "products" = confirmed, live in PIM.
	// "pending_imports" = awaiting user review, not yet in PIM.
	// Defaults to "products" when empty (backwards compatible).
	SourceCollection string    `firestore:"source_collection,omitempty" json:"source_collection,omitempty"`
...
type ChannelConfig struct {
	...
	Products ChannelProductConfig `firestore:"products" json:"products"`

	// Temu-specific listing defaults
	TemuDefaults *TemuDefaultsConfig `firestore:"temu_defaults,omitempty" json:"temu_defaults,omitempty"`
...
// TemuDefaultsConfig holds per-credential Temu listing defaults.
// Values here pre-fill the Temu listing form and can be overridden per listing.
type TemuDefaultsConfig struct {
	FulfillmentType    int    `firestore:"fulfillment_type" json:"fulfillment_type"`
	ShipmentLimitDay   int    `firestore:"shipment_limit_day" json:"shipment_limit_day"`
	ShippingTemplateID string `firestore:"shipping_template_id" json:"shipping_template_id"`
	OriginRegion1      string `firestore:"origin_region1" json:"origin_region1"`
	OriginRegion2      string `firestore:"origin_region2" json:"origin_region2"`
}

// ChannelProductConfig controls product import behaviour for this channel.
type ChannelProductConfig struct {
	// ImportEnabled: when true, connecting or reconnecting this channel
	// automatically downloads products into the PIM. Defaults to true.
	ImportEnabled bool `firestore:"import_enabled" json:"import_enabled"`
}
...
func DefaultChannelConfig() ChannelConfig {
	return ChannelConfig{
		...
		Products: ChannelProductConfig{
			ImportEnabled: true, // auto-import on by default
		},
...
type ImportProductsRequest struct {
	...
	PendingReview      bool     `json:"pending_review,omitempty"`     // Auto-connect import: stage products for review rather than live creation
}
