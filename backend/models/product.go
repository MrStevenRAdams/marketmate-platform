...
type Product struct {
	...
	// Shipping & Dispatch
	// ShippingTemplate links to a shipping_templates document for label layout.
	ShippingTemplate string `json:"shipping_template,omitempty" firestore:"shipping_template,omitempty"`
	// LabelsPerShipment is the number of labels generated per dispatch. Default 1.
	// Set > 1 for multi-box shipments (e.g. flat-pack furniture) — each parcel gets a unique Evri barcode.
	LabelsPerShipment int `json:"labels_per_shipment,omitempty" firestore:"labels_per_shipment,omitempty"`
	// DefaultCarrier is the preferred carrier code (e.g. "evri") for dispatching this product.
	DefaultCarrier string `json:"default_carrier,omitempty" firestore:"default_carrier,omitempty"`
	// DefaultService is the preferred carrier service code (e.g. "PARCEL_SIGNATURE").
	DefaultService string `json:"default_service,omitempty" firestore:"default_service,omitempty"`
	// IncludeReturnLabel — when true, every dispatch for this product generates both an outbound
	// and a return label simultaneously.
	IncludeReturnLabel bool `json:"include_return_label,omitempty" firestore:"include_return_label,omitempty"`
	// CustomsProfileID links to a customs_profiles document for international shipments.
	CustomsProfileID string `json:"customs_profile_id,omitempty" firestore:"customs_profile_id,omitempty"`
