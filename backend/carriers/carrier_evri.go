...
func (a *EvriAdapter) mapShipmentRequest(req ShipmentRequest) (evriRoutingEntry, error) {
	...
	return evriRoutingEntry{
		...
		CountryCode:     normaliseEvriCountry(to.Country),
		Phone:           to.Phone,
		Email:           to.Email,
...
// normaliseEvriCountry maps country codes that Evri does not accept natively
// to the code Evri requires in the API request.
//
// Per Evri routing spec (v1.6.14, p.78):
//   "When creating shipments to the Channel Islands and Isle of Man,
//    the country code clients must populate in the API request is 'GB',
//    rather than 'JE' / 'GG' / 'IM'."
func normaliseEvriCountry(iso2 string) string {
